# SSDBM 2026 Website — Information Needed

Please fill in the blanks directly. Items already answered are marked.

---

## Needed for Website Launch

### Conference Basics

- Dates: **August 11–13, 2026**
- Host: **San Diego Supercomputer Center (SDSC), UC San Diego**
- Location: **San Diego, California, USA**
- Building / room: **SDSC Auditorium**
- Address: **9836 Hopkins Dr, La Jolla, CA 92037**
- Visitor info: **https://www.sdsc.edu/contact.html**

### Organizing Committee

- General Chairs: **Jay Lofstead, Christine Kirkpatrick**
- Program Co-chairs: **Hari Devarajan, Diana Moise**
- Short and Demo Paper Co-chairs:
- Student Posters Competition Co-chairs:
- Proceedings Chair:
- Web and Publicity Chair: **Jaime Cernuda Garcia**

### Steering Committee

Current list (carried over from 2025 — please confirm or update):

- Arie Shoshani, Lawrence Berkeley National Lab (Chair Emeritus)
- John Wu, Lawrence Berkeley National Lab (Chair)
- Suren Byna, The Ohio State University
- Shadi Ibrahim, Inria, Rennes
- Carl Kesselman, University of Southern California
- Yongluan Zhou, University of Copenhagen

Any changes? (add/remove names):

### Supporters

- Host logos: **SDSC + UCSD logos added**
- Any other institutional supporters? (names, logos, URLs):
- Sponsorship contact person (for "Call for Sponsors" page):

---

## Needed for Call for Papers

### Submission Dates

- Abstract submission deadline (main track):
- Paper submission deadline (main track):
- Short & Demo paper submission deadline:
- Poster submission deadline:
- Notification of acceptance (papers):
- Notification of acceptance (short/demo):
- Notification of acceptance (posters):
- Camera-ready deadline:
- Author registration deadline:

### Submission System

- EasyChair URL (or alternative):
- Conference contact email:

### Topics & Review

- Any changes to the topics of interest? (currently carried over from 2025):
- Review policy: single-blind (same as 2025) or double-blind?

---

## Needed Later (can be added incrementally)

### Program Committees

- Full Papers PC members (names + affiliations):
- Short and Demo Papers PC members (names + affiliations):
- Posters Committee members (names + affiliations):

### Keynote Speakers

For each speaker: name, affiliation, talk title, abstract, bio, photo.

### Venue & Travel

- Venue photos (for homepage and venue page):
- Recommended hotels: **3 listed from [NSDF AHM 2025](https://nationalsciencedatafabric.org/nsdf-ahm-2025-01) (same venue) — committee to confirm or expand**
- Visa invitation letter contact: **Jay Lofstead (lofsteadjaywork@gmail.com)**

### Registration

- Registration platform (Qualtrics, Eventbrite, etc.):
- Registration fees (author / regular / student, early bird vs. late):
- What's included (meals, social events, etc.):
- Registration policies (refund, substitution, deadlines):
- Social events (e.g. welcome reception location):

### Other

- Conference logo or banner (if different from standard SSDBM logo):
- Any special workshops or co-located events:
- Travel awards / student grants:
- Any new paper tracks or categories:
- ACM APC rates for 2026 (check with ACM when available):

---

## Notes

- Topics of interest were carried over from 2025 — worth reviewing whether to add anything (LLMs for science, quantum computing, etc.).
- ACM ICPS publishing model and camera-ready instructions are standard and already on the site, but APC rates may change year to year.
- Hotel list was sourced from the [NSDF All-Hands Meeting 2025](https://nationalsciencedatafabric.org/nsdf-ahm-2025-01) (same SDSC venue) — committee should confirm which hotels to recommend.
